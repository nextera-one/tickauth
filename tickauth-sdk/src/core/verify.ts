...
import type { TickAuthProof, VerifyResult, TickAuthClearance } from './types';
import { verifyEd25519 } from './crypto';
import { serializeChallenge, getCurrentTick } from './challenge';
import { ReplayGuard, getDefaultReplayGuard } from './replay-guard';
...
export async function verifyProof(
  ...
  if (requireTrustedKey && !publicKeyHex) {
    return {
      ok: false,
      error: 'INVALID_PROOF',
      message: 'No trusted public key provided. Refusing to trust embedded key.',
    };
  }
  ...
  const message = new TextEncoder().encode(
    JSON.stringify({
      challenge: serializeChallenge(proof.challenge),
      tick: proof.tick,
      signedAt: proof.signedAt,
    }),
  );

  const sigValid = await verifyEd25519(message, proof.sig.sigHex, publicKeyHex ?? proof.sig.publicKeyHex);
  if (!sigValid) {
    const capsule = createCapsule({ proof, status: 'denied', reason: 'INVALID_SIGNATURE' });
    return {
      ok: false,
  ...
  if (proof.tick < window.tickStart - drift) {
    const capsule = createCapsule({ proof, status: 'expired', reason: 'OUTSIDE_WINDOW_EARLY' });
    return {
      ok: false,
  ...
  if (proof.tick > window.tickEnd + drift) {
    const capsule = createCapsule({ proof, status: 'expired', reason: 'OUTSIDE_WINDOW_LATE' });
    return {
      ok: false,
      error: 'OUTSIDE_WINDOW',
  ...
  if (timeDiff > window.tickEnd - window.tickStart + drift) {
    const capsule = createCapsule({ proof, status: 'denied', reason: 'DRIFT_EXCEEDED', driftApplied: timeDiff });
    return {
      ok: false,
  ...
  if (!options.skipReplayCheck) {
    ...
    if (!replayGuard.check(nonce)) {
      const capsule = createCapsule({ proof, status: 'replay_rejected', reason: 'NONCE_ALREADY_USED' });
      return {
        ok: false,
...
export function isProofInWindow(
  ...
  return proof.tick >= window.tickStart - drift && proof.tick <= window.tickEnd + drift;
}
